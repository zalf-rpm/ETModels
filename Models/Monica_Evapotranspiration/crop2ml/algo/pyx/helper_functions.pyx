from math import log, cos, sin, asin, pi, sqrt, tan, acos, exp
...
def calc_reference_evapotranspiration(float height_nn,
    ...
    arg_SHA = bound(-1.0, -tan(latitude * pi / 180.0) * tan(declination * pi / 180.0),
                    1.0)
    cdef float SHA
    SHA = acos(arg_SHA)
    ...
    relative_shortwave_radiation = min(global_radiation / clear_sky_solar_radiation, 1.0) if clear_sky_solar_radiation > 0 else 1.0
...
def e_reducer_1(float pwp,
                ...
                float evaporation_reduction_method,
                float xsa_critical_soil_moisture):

    cdef float e_reduction_factor
    e_reduction_factor = 0
