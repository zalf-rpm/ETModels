...
void ReferenceEvapotranspiration::Init(ETState &s, ETState &s1, ETRate &r, ETAuxiliary &a, ETExogenous &ex)
{
    s.net_radiation = 0.0;
...
void ReferenceEvapotranspiration::Calculate_Model(ETState &s, ETState &s1, ETRate &r, ETAuxiliary &a, ETExogenous &ex)
{
    ...
    s.reference_evapotranspiration = (0.408 * saturated_vapour_pressure_slope * s.net_radiation + (psycrometer_constant * (900.0 / (ex.mean_air_temperature + 273.0)) * wind_speed_2m * saturation_deficit)) / (saturated_vapour_pressure_slope + (psycrometer_constant * (1.0 + (surface_resistance / 208.0 * wind_speed_2m))));
    if (s.reference_evapotranspiration < 0.0) {
        s.reference_evapotranspiration = 0.0;
    }
}
