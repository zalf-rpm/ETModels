...
void Evapotranspiration::Init(EvapotranspirationCompState &s, EvapotranspirationCompState &s1, EvapotranspirationCompRate &r, EvapotranspirationCompAuxiliary &a, EvapotranspirationCompExogenous &ex)
{
    ...
    s.crop_transpiration = std::vector<double>(no_of_soil_moisture_layers);
    ...
    s.crop_transpiration = std::move(std::vector<double>(no_of_soil_moisture_layers, 0.0));
...
void Evapotranspiration::Calculate_Model(EvapotranspirationCompState &s, EvapotranspirationCompState &s1, EvapotranspirationCompRate &r, EvapotranspirationCompAuxiliary &a, EvapotranspirationCompExogenous &ex)
{
    ...
    if (s.developmental_stage > 0) {
        ...
        potential_evapotranspiration = s.crop_remaining_evapotranspiration;
        evaporated_from_intercept = s.crop_evaporated_from_intercepted;
    }
    else {
        if (ex.external_reference_evapotranspiration < 0.0) {
        ...
        potential_evapotranspiration = s.reference_evapotranspiration * s.kc_factor;
    }
    s.actual_evaporation = 0.0;
    s.actual_transpiration = 0.0;
    ...
    if (potential_evapotranspiration > 0.0) {
        ...
        if (s.surface_water_storage > 0.0) {
            ...
            potential_evapotranspiration = potential_evapotranspiration * 1.1 / s.kc_factor;
            if (s.snow_depth > 0.0) {
                s.evaporated_from_surface = 0.0;
            }
            ...
            potential_evapotranspiration = potential_evapotranspiration * s.kc_factor / 1.1;
        }
        if (potential_evapotranspiration > 0.0) {
            for (i=0; i!=no_of_soil_layers; i+=1) {
                ...
                if (s.developmental_stage > 0) {
                    if (s.percentage_soil_coverage >= 0.0 && s.percentage_soil_coverage < 1.0) {
                        s.evaporation[i] = (1.0 - s.percentage_soil_coverage) * eReducer * potential_evapotranspiration;
                    }
                    else if (s.percentage_soil_coverage >= 1.0) {
                        s.evaporation[i] = 0.0;
                    }
                    if (s.snow_depth > 0.0) {
                        s.evaporation[i] = 0.0;
                    }
                    s.transpiration[i] = s.crop_transpiration[i];
                    if (evaporation_from_surface) {
                        s.transpiration[i] = s.percentage_soil_coverage * eReducer * potential_evapotranspiration;
                    }
                }
                else {
                    if (s.snow_depth > 0.0) {
