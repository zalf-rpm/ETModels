...
double ETComponent::getstomata_resistance() { return this->stomata_resistance; }
...
void ETComponent::setreference_albedo(double _reference_albedo) {
    _ReferenceEvapotranspiration.setreference_albedo(_reference_albedo);
}

void ETComponent::setstomata_resistance(double _stomata_resistance) {
    _ReferenceEvapotranspiration.setstomata_resistance(_stomata_resistance);
}

void ETComponent::setlatitude(double _latitude) {
    _ReferenceEvapotranspiration.setlatitude(_latitude);
}
...
void ETComponent::Calculate_Model(ETState& s, ETState& s1, ETRate& r, ETAuxiliary& a, ETExogenous& ex) {
    ...
    ex.reference_evapotranspiration = s.reference_evapotranspiration;
...
void ETComponent::Init(ETState& s, ETState& s1, ETRate& r, ETAuxiliary& a, ETExogenous& ex) {
    ...
    _ReferenceEvapotranspiration.Init(s, s1, r, a, ex);
    _PotentialEvapotranspiration.Init(s, s1, r, a, ex);
}
